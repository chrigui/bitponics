...
Strategy.prototype.authenticate = function(req) {
  ...
  this._getUser(publicKey, function(err, user, matchingKey) {
    ...
    var deviceId = req.url.substr(req.url.indexOf("api/devices/") + "api/devices/".length, 12);
    if (matchingKey.device !== deviceId){
    ...
    winston.info(req.method + ' ' + req.url + ' HTTP/' + req.httpVersion + req.rawBody + headerSaltValue);
